...
func (w *wallet) BatchWallet(ctx context.Context, passphrases []string, batchPassphrase string) error {
	...
	accounts, err := w.obtainAccountsForBatch(ctx, passphrases)
	if err != nil {
		return err
	}
...
func (w *wallet) obtainAccountsForBatch(ctx context.Context, passphrases []string) ([]*account, error) {
	...
	for data := range w.store.RetrieveAccounts(w.ID()) {
		if account, err := deserializeAccount(w, data); err == nil {
			...
			if !unlocked {
				return nil, fmt.Errorf("unable to decrypt account %q with supplied passphrases", account.name)
			}
...
// batchPresent returns true if the batch is present.
func (w *wallet) batchPresent() bool {
	w.batchMutex.Lock()
	defer w.batchMutex.Unlock()

	return w.batch != nil && len(w.batch.entries) > 0
}
